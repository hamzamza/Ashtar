...
const getkey = (node) => `x:${node.position.x},y:${node.position.y}`
...
constructor(nodes,edges){
    ...
    this.edges.forEach(edge => this.graph.addEdge(getkey(edge.vortex1), getkey(edge.vortex2),Math.sqrt(Math.pow(edge.vortex1.position.x-edge.vortex2.position.x,2)+Math.pow(edge.vortex1.position.y-edge.vortex2.position.y,2))) );
}
